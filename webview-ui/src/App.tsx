...
import { Tldraw, useEditor, createShapeId, toRichText, LicenseManager } from '@tldraw/tldraw';
  ...
  useEffect(() => {
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (message?.command === 'addPointer' && editor) {
        const { filePath, startLine, endLine, preview } = message.data;
        const fileName = (filePath || '').split('/').pop() || (filePath || '').split('\\').pop() || 'file';
        const label = `${fileName}  L${startLine}-${endLine}`;

        try {
          editor.createShapes([{
            id: createShapeId(),
            type: 'geo',
            x: Math.max(80, (window.innerWidth / 2) - 75),
            y: Math.max(80, (window.innerHeight / 2) - 55),
            props: {
              geo: 'ellipse',
              color: 'light-blue',
              labelColor: 'black',
            ...
            meta: { filePath, startLine, endLine, preview },
          }]);
        } catch (error) {
